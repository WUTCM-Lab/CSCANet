...
def loadData():
    # 读入数据
    data = sio.loadmat('../data/WHU-Hi-HanChuan/WHU_Hi_HanChuan.mat')['WHU_Hi_HanChuan']
    labels = sio.loadmat('../data/WHU-Hi-HanChuan/WHU_Hi_HanChuan_gt.mat')['WHU_Hi_HanChuan_gt']
    return data, labels
...
def splitTrainTestSet(X, y, test_ratio, randomState=345):
...
def train(train_loader, epochs):
    ...
    # 网络放到GPU上
    net = CSCANet.CSCANet().to(device)
    # 交叉熵损失函数
    # criterion =LabelSmoothingCrossEntropy()
    criterion = nn.CrossEntropyLoss()
    # criterion =lpolyloss.PolyLoss()
    # 初始化优化器
...
def acc_reports(y_test, y_pred_test):
    ...
    target_names = ['Strawberry', 'Cowpea', 'Soybean', 'Sorghum'
        , 'Water-spinach', 'Watermelon', 'Greens',
                    ...
                    'Plastic', 'Bare-soil', 'Roads', 'Bright-object',
                    'Water']
    # WHU_HANCHUAN
...
if __name__ == '__main__':
    ...
    for index_iter in range(10):
        ...
        net, device = train(train_loader, epochs=150)
        print('cliqueNet parameters:', sum(param.numel() for param in net.parameters()))
        # 只保存模型参数
        torch.save(net.state_dict(),'./cla_pth/classification_report_HC_'+ str(index_iter) + '.pth')
        ...
        get_cls_map.get_cls_map(net, device, all_data_loader, y_all, index_iter)
    record.record_output(
        OA, AA, KAPPA, ELEMENT_ACC, TRAINING_TIME, TESTING_TIME,
        './report/' + 'CSCANet_HC.txt')
